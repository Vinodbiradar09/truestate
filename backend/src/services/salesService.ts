...
function buildQuery(filters: SalesFilters) {
    ...
    let params: any[] = [];
    let paramIndex = 1;
    let searchParamIndices: number[] = [];
    if (filters.search && filters.search.trim() !== "") {
        const searchTerm = filters.search.trim();
        searchParamIndices = [paramIndex, paramIndex + 1, paramIndex + 2];
        
        conditions.push(
            `(
                customer_name ILIKE $${paramIndex} 
                OR customer_name ILIKE $${paramIndex + 1}
                OR customer_name ILIKE $${paramIndex + 2}
                OR phone_number ILIKE $${paramIndex}
                OR phone_number ILIKE $${paramIndex + 1}
                OR phone_number ILIKE $${paramIndex + 2}
            )`
        );
        params.push(
            searchTerm,           
    ...
    if (filters.search && filters.search.trim() !== '' && searchParamIndices.length > 0) {
        orderByClause = `ORDER BY 
            CASE
                WHEN LOWER(customer_name) = LOWER($${searchParamIndices[0]}) THEN 1
                WHEN LOWER(phone_number) = LOWER($${searchParamIndices[0]}) THEN 1
                WHEN LOWER(customer_name) LIKE LOWER($${searchParamIndices[1]}) THEN 2
                WHEN LOWER(phone_number) LIKE LOWER($${searchParamIndices[1]}) THEN 2
                WHEN LOWER(customer_name) LIKE LOWER($${searchParamIndices[2]}) THEN 3
                WHEN LOWER(phone_number) LIKE LOWER($${searchParamIndices[2]}) THEN 3
                ELSE 4
            END,
            `;
    } else {
